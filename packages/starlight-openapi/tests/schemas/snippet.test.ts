...
const expectedDefaultOperationClients = Object.entries(defaultOperationClients).flatMap(([target, clients]) =>
  clients.map((client) => ({ target, client })),
)

const expectedDefaultOperationClient = { target: 'shell', client: 'curl' }
