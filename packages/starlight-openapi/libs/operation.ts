...
export function getOperationsByTag({ config, document }: Schema) {
  ...
  if (document.tags) {
    ...
    const operationsByTagArray = operationsByTag
      .entries()
      .toArray()
      .toSorted(([tagA], [tagB]) => {
        const orderA = orderedTags.get(tagA)?.index ?? Infinity
        const orderB = orderedTags.get(tagB)?.index ?? Infinity
...
export function isPathItemOperation<TMethod extends OperationHttpMethod>(
  ...
  return Object.hasOwn(pathItem, method)
}
...
function getRequestServerURL(serverObject: ServerObject): string {
  ...
  return new URL(url, defaultServerOrigin).href
}
