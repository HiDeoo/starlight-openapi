...
export const SnippetsSchema = z
  .object({
    /**
     * Operation snippets are code examples shown at the top of operation pages, with a picker to switch between
     * languages and clients, that demonstrate how to execute an operation in a specific environment.
     * Operation snippets can be autogenerated from the OpenAPI document or authored using the `x-codeSamples` OpenAPI
     * extension.
     *
    ...
    /**
     * Response snippets are code examples shown in response sections on operation pages.
     * They can be autogenerated from the response schema or authored directly in the OpenAPI document.
     *
     * This option controls whether autogenerated response snippets are available in response sections for JSON-like
  ...
  .default({ operation: defaultOperationConfig, response: true })
